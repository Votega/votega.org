...
from your browser. Instead, automated workflows run daily, pull data from trusted public sources,
...
district, term dates, and official photo — is pulled from the Congress.gov v3 API daily.
...
- **Term start dates:** Not consistently available from Open States for Georgia. We maintain a manual overrides file to fill in known values.
- **Freshness:** Updated daily via an automated GitHub Actions workflow.
...
**[github.com/Votega/ga-legislators](https://github.com/Votega/ga-legislators)**
...
The data is published as `data/all.json` and follows the same schema as our internal
`ga-members.json` file. See the repository README for field definitions.
...
into the site on the next daily update.
...
- **No real-time API calls from your browser.** All data is pre-fetched and served as static files. This means no API key exposure, no CORS issues, and fast page loads.
- **No tracking or analytics beyond standard page metrics.** We do not build profiles of visitors or sell data.
- **No user accounts or form submissions.** The site is read-only.

---

## Corrections and Feedback

See an error? Have a data question? Reach out at [admin@votega.org](mailto:admin@votega.org)
