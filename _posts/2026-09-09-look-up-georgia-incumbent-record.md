...
> **Jump in:** See how members voted with [Party Unity]({{ '/ga-party-unity.html' | relative_url }}), track control of the legislature with the [Majority Tracker]({{ '/ga-majority-tracker.html' | relative_url }}), and browse [Congressional Trades]({{ '/ga-congress-trades.html' | relative_url }}). Or start at [Find My Reps]({{ '/find-my-reps' | relative_url }}).
...
- **[Majority Tracker]({{ '/ga-majority-tracker.html' | relative_url }})** shows who actually controls each chamber and how close the balance is — useful context for why a single state House or Senate race can matter far more than its size suggests.

Prefer to start from the bills themselves? The [Georgia Bills tracker]({{ '/ga-bills.html' | relative_url }}) covers legislation from the current session, and each member's page ties their votes back to the bills that produced them.

## What Georgia's congressional delegation has been trading
...
3. For members of Congress, check their **[trades]({{ '/ga-congress-trades.html' | relative_url }})**.
4. Then preview your full ballot with the **[Sample Ballot tool]({{ '/sample-ballot' | relative_url }})** and make your plan to vote before **November 3**.

## Built on open data — check our work

None of this is opinion. Every figure traces back to a primary source: the Georgia General Assembly, the U.S. House and Senate, the Federal Election Commission, and the Georgia Secretary of State. We publish the underlying datasets openly under a Creative Commons license on our [Open Data page]({{ '/open-data' | relative_url }}) — so journalists, researchers, and curious voters can verify every number and build on it.

{: .box-success}
Accountability shouldn't be hard. Look up your incumbents, share what you find, and vote your full ballot on **November 3, 2026**.

*VoteGA is a nonpartisan, volunteer civic-data project. Records are compiled from official public sources and refreshed automatically; see [About the Data]({{ '/about-the-data' | relative_url }}) for methodology and sources.*
